...
public class Main {
  ...
  public void run(RuntimeSettings runtimeSettings) {
    ...
    ImageProcessor cubeImageProcessor = CubeImageProcessorFactory.CreateImageProcessor(publishingTable);
   // ImageProcessor redBallImageProcessor = RedBallImageProcessorFactory.CreateImageProcessor(publishingTable);
    ...
    while (!Thread.currentThread().isInterrupted()) {
      ...
      //  redBallImageProcessor.processAsync(inputImage);
        cubeImageProcessor.processAsync(inputImage);
        imagePump.pumpAsync();

        // Await image processing to finsh
       // redBallImageProcessor.awaitProcessCompletion();
        cubeImageProcessor.awaitProcessCompletion();

        // Annotate the image
       // outputImage = redBallImageProcessor.annotate(inputImage);
        outputImage = cubeImageProcessor.annotate(outputImage);
